...
evidence_generate_component_artifact_sbom() {
...
evidence_attach_component_artifact_sbom() {
...
#   8 att_tag_ref          registry/repo:tag (string, may be empty)
...
evidence_make_oci_attestation_record() {
  ...
  local subject_digest_ref="$5" predicate_path="$6" predicate_type="$7"
  local att_tag_ref="$8" att_digest_ref="$9" att_mediaType="${10}"
  local att_size="${11}" att_pushed_at="${12}"

  [[ -n "$subject_digest_ref" ]] || die "evidence_make_oci_attestation_record: missing subject_digest_ref"
  [[ -n "$predicate_path" ]] || die "evidence_make_oci_attestation_record: missing predicate_path"
  [[ -f "$predicate_path" ]] || die "evidence_make_oci_attestation_record: predicate file not found: $predicate_path"
  ...
  pred_size="$(wc -c <"$predicate_path" | tr -d '[:space:]')"
  now="$(date -u +%Y-%m-%dT%H:%M:%SZ)"

  # Stable dedupe key: same subject + same predicate type + same predicate content => same key
  ...
  jq -c -n \
    ...
    '{
      ...
      oci: {
        tag_ref: (if ($att_tag_ref|length)>0 then $att_tag_ref else null end),
...
evidence_attest_artifact_predicate() {
  ...
  ev="$(evidence_make_oci_attestation_record \
        ...
        "$tag_ref" "$digest_ref" "$mediaType" "$size" "$pushed_at")"
